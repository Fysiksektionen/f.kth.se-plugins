<?php
/*
Plugin Name: wpCAS with LDAP (Fysiksektionen Platinum Edition)
Plugin URI: http://wordpress.org/extend/plugins/wpcas-w-ldap/
Version: 1.0
Description: Fixar KTH inlogg och släpper bara in fysiker. (moddad av Leo Fidjeland & Emil Lundberg)
...
if ($wpcasldap_use_options['server_hostname'] == '' ||
		/*$wpcasldap_use_options['server_path'] == '' ||*/ //<-- Fix 2012-03-08 by Calle Svensson <carlsven@kth.se>. Needed to prevent url to include double forward slash.
	...
	*/
	...
	function authenticate() {
		...
		if( phpCAS::isAuthenticated() ){
			// CAS was successful
			$newuser = get_ldap_user(phpCAS::getUser());
			$userdata = $newuser->get_user_data();
			if ( $user = get_userdatabylogin($userdata['user_login'])){ // user already exists
...
function wpcasldap_nowpuser($newuserid) {
	...
	if(!$userdata['user_email']){
		$userdata['user_email'] = $userdata['user_login'].'@'.$wpcasldap_use_options['email_suffix'];
	}
	if (!function_exists('wp_insert_user'))
		include_once ( ABSPATH . WPINC . '/registration.php');
	
	if(!is_fysiker($userdata['user_login'])){
		wp_die('Fysiksektionens hemsida är endast till för Fysiker.<br /><br />Om du är fysiker och ändå ser det här meddelandet har någonting gått hemskt fel! Maila <a href="mailto:weber@f.kth.se">weber@f.kth.se</a> så löser han problemet.','Åtkomst Nekad');
	}
	...
	if(!$ds) {
		...
		if(!ldap_set_option($ds, LDAP_OPT_PROTOCOL_VERSION, 3)) {
			...
			if(!$bind) {
				...
				$search = ldap_search($ds, $wpcasldap_use_options['ldapbasedn'], "ugkthid=$uid");
...
function is_fysiker($user) {
  ...
  if(isset($user) && $user) {
    ...
    for($x=0;$read[$x] = utf8_encode(fgets($handle));$x++) {
      if(substr($read[$x],0,strlen($user)+1)==$user.":") {
    ...
    pclose($handle);
    if(isset($exakt)) {##Bara intresserade av exakta matchar nu
